...
type ExpandEventsInternal<K extends keyof T, T> = K extends string ? {
  addEventListener(type: K, listener: (this: HTMLElement, ev: T[K]) => any, options?: boolean | AddEventListenerOptions): void;
  removeEventListener(type: K, listener: (this: HTMLElement, ev: T[K]) => any, options?: boolean | AddEventListenerOptions): void;
} : never;
 ...
 */
export type TypedEventConstructor<X, T> = (new () => (ExpandEvents<T> & X));
